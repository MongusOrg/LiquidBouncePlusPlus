...
@ModuleInfo(name = "HackerDetector", description = "Detect Hackers. (fdp skid)", category = ModuleCategory.PLAYER)
...
//        }
    ...
    private fun flag(type: String,vl: Int,data: HackerData,msg: String){
        ...
        if(data.vl>vlValue.get()){
            ...
            //chat("§f${data.player.name} might be using $use")
            LiquidBounce.hud.addNotification(Notification("${data.player.name} might be using $use", Notification.Type.WARNING, 3000L))
            data.vl=-vlValue.get()
