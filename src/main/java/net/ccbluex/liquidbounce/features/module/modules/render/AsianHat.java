...
public class AsianHat extends Module {
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (hatRotation.get() && RotationUtils.serverRotation != null) {
            GlStateManager.rotate(RotationUtils.serverRotation.getYaw() / (180F / (float)Math.PI), 0, 1, 0);
            GlStateManager.rotate(RotationUtils.serverRotation.getPitch() / (180F / (float)Math.PI), 1, 0, 0);
        }

        worldrenderer.begin(GL11.GL_POLYGON, DefaultVertexFormats.POSITION_COLOR);

        // main section
        worldrenderer.pos(0, 0, 0).color(r, g, b, al).endVertex();
