...
class ToggleSwitch(var accentColor: Color = Color(0, 140, 255)) {
    ...
    fun onDraw(x: Float, y: Float, width: Float, height: Float, bgColor: Color) {
        smooth = (smooth + (if (state) 0.2F else - 0.2F) * RenderUtils.deltaTime * 0.0095F).coerceIn(0F, 1F)
        val borderColor = BlendUtils.blendColors(floatArrayOf(0F, 1F), arrayOf(Color(160, 160, 160), accentColor), smooth)
        val borderColor = BlendUtils.blendColors(floatArrayOf(0F, 1F), arrayOf(bgColor, accentColor), smooth)
        val borderColor = BlendUtils.blendColors(floatArrayOf(0F, 1F), arrayOf(Color(160, 160, 160), bgColor), smooth)
