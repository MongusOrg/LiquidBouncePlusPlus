...
class IntElement(val savedValue: IntegerValue): ValueElement<Int>(savedValue) {
    ...
    override fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, bgColor: Color): Float {
        val valueDisplay = 30F + Fonts.font40.getStringWidth(savedValue.maximum.toString())
        val maxLength = Fonts.font40.getStringWidth(savedValue.maximum.toString())
        val minLength = Fonts.font40.getStringWidth(savedValue.minimum.toString())
        ...
        if (dragged)
            savedValue.set((savedValue.minimum + (savedValue.maximum - savedValue.minimum) / sliderWidth * (mouseX - startPoint)).toInt())
        val currLength = Fonts.font40.getStringWidth(savedValue.get().toString())
        Fonts.font40.drawString(value.name, x + 10F, y + 10F - Fonts.font40.FONT_HEIGHT / 2F, -1)
        Fonts.font40.drawString(savedValue.maximum.toString(), 
                                x + width - 10F - maxLength - valueDisplay, 
                                y + 10F - Fonts.font40.FONT_HEIGHT / 2F, -1)
        Fonts.font40.drawString(savedValue.minimum.toString(), 
                                x + width - 30F - sliderWidth - maxLength - minLength - valueDisplay, 
                                y + 10F - Fonts.font40.FONT_HEIGHT / 2F, -1)
        slider.setValue(savedValue.get().toFloat(), savedValue.minimum.toFloat(), savedValue.maximum.toFloat())
        ...
        Fonts.font40.drawString(savedValue.get().toString(), x + width + 6F - valueDisplay, y + 10F - Fonts.font40.FONT_HEIGHT / 2F, -1)
    ...
    override fun onClick(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float) {
        ...
        if (MouseUtils.mouseWithinBounds(mouseX, mouseY, x + width - 5F - valueDisplay, y + 2F, x + width + 3F - valueDisplay, y + 18F))
            savedValue.set(savedValue.get() - 1)
