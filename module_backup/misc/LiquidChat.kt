...
@ModuleInfo(name = "IRC", description = "Allows you to chat with other LiquidBounce and EvoBounce users.", category = ModuleCategory.MISC)
            ...
            when (packet) {
                is ClientMessagePacket -> {
                    if (mc.thePlayer == null) {
                        ClientUtils.getLogger().info("[IRC] ${packet.user.name}: ${packet.content}")
                        return
                    }

                    val chatComponent = ChatComponentText("§7[§6§lIRC§7] §9${packet.user.name}: ")
                ...
                is ClientPrivateMessagePacket -> ClientUtils.displayChatMessage("§7[§6§lIRC§7] §c(Private)§9 ${packet.user.name}: §7${packet.content}")
